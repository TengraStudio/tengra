 ...
 * This script finds the latest installed Visual Studio and sets node-gyp env vars.
 */
...
function setMSVSVersion(version) {
    try {
        // Set environment variables that node-gyp reads directly.
        ...
        console.error(`✗ Failed to set Visual Studio build environment:`, err.message);
        return false;
    }
}
...
function main() {
    ...
    if (setMSVSVersion(latest.version)) {
        ...
        console.error('\n❌ Failed to set Visual Studio build environment');
        process.exit(1);
    }
}
