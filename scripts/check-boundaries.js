...
const rules = {
    'src/renderer': {
        forbidden: [
            {
                pattern: /^\s*import\s+(?!type\b).*from ['"]electron['"]/m,
                message: 'Direct electron import in renderer',
            },
    ...
    'src/main': {
        forbidden: [
            {
                pattern: /^\s*import\s+(?!type\b).*from ['"]react['"]/m,
                message: 'React import in main process',
            },
...
function checkFile(filePath, forbidden) {
    ...
    // Imports are expected at the top; restricting scan avoids false positives inside template literals.
    const contentHead = content.split('\n').slice(0, 120).join('\n');
    const relativePath = path.relative(ROOT_DIR, filePath);
    const fileViolations = [];

    for (const rule of forbidden) {
        if (rule.pattern.test(contentHead)) {
            fileViolations.push({
                file: relativePath,
