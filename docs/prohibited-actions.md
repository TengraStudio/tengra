...
- **NO SLANG/NOISE**: Do not write low-signal, sarcastic, or exaggerated assistant text.
- **NO RULE THEATER**: Do not tell the user which internal rules were followed.
- **NO DIRECT TAILWIND UTILITIES IN RENDERER JSX**: Utility tokens for typography, spacing, layout, color, radius, border, shadow, ring, or transition are forbidden.
- **NO MULTI-CSS DRIFT**: Do not create or import extra renderer CSS files; use only `src/renderer/index.css`.
...
- **NO UNVERIFIED DELIVERY**: Do not present a change as complete without explicit verification status.
