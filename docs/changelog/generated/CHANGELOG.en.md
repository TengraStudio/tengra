...
### QUALITY-016: Code Intelligence Service Reduced To Indexing Orchestration

- **Type**: refactor
- **Status**: completed
- **Summary**: Refactored CodeIntelligenceService into a slimmer orchestrator that keeps project indexing in-service while delegating scanners, navigation, rename flows, documentation generation, and quality analysis to extracted utilities.

- `CodeIntelligenceService` now focuses on indexing coordination, semantic fragment storage, progress events, and symbol analytics.
- File scanning, symbol parsing/navigation, rename execution, documentation generation, and quality scanning now route through the dedicated `code-intelligence/*` utility modules.
- Preserved the existing IPC-facing API surface while cutting `code-intelligence.service.ts` down from 1200+ lines to a much smaller coordinator.

### QUALITY-017: Copilot Service Split Into Focused Modules

- **Type**: refactor
- **Status**: completed
- **Summary**: Replaced the monolithic Copilot service with a thin facade that delegates token management, rate limiting, request building, response parsing, and API calls to dedicated modules.

- `CopilotService` now owns only shared state, lifecycle hooks, configuration setters, and the public chat/token facade methods.
- Token refresh, rate limiting, request construction, response parsing, and gateway communication now flow through the extracted Copilot helper classes.
- Preserved `isConfigured()` and VS Code version refresh behavior while reducing `copilot.service.ts` from 1000+ lines to a compact facade.

### Council Scenario Tests Finalized as Split Suites

- **Type**: refactor
- **Status**: completed
- **Summary**: Completed QUALITY-055 by removing the obsolete monolithic council scenario suite and keeping the split scenario files as the single source of truth.

- **QUALITY-055**: Deleted the legacy `council-scenarios.test.ts` file that still carried the `max-lines-per-function` lint suppression.
- **Canonical Split Suites**: Kept the shared helpers module plus the three scenario-focused test files as the active council scenario coverage.
- **Maintenance**: Removed duplicate test execution and aligned the test directory with the intended smaller-file layout.
...
- **Build**: Fixed various build/lint errors and verified project integrity with npm run type-check.
...
- **Type**: perf
- **Status**: completed
- **Summary**: Lazy-loaded settings tabs, memoized MCP ToolCard, fixed store mutation patterns, converted sync I/O to async, and optimized hot-path iteration in orchestrator.

- **MCPStore ToolCard**: Wrapped in React.memo to prevent re-renders of 24+ cards.
- **voice.store**: Replaced .push()/.splice() mutations with immutable array operations for proper change detection.
- **loading-analytics.store**: Replaced filter+reduce with single-pass loop in recalculateAverageDuration.
...
- Activated MCP marketplace settings tab and linked browser/installed/compare flows with cards, detail view, install wizard, ratings, and comparison matrix
...
- **Type**: feature
- **Status**: completed
- **Summary**: Added a branch-switch popover in the workspace command strip so users can browse and switch git branches without leaving the editor.

- Made the command strip branch label clickable and opened a popover listing available branches
- Loaded current branch and branch list from the active project repository with loading and empty states
- Added branch checkout action with success/error notifications and switching-state feedback
...
- **Performance Batch #2**: Memoized MessageList action handlers for stable row callbacks and changed project sorting flow to sort once per active sort mode, then filter the sorted result.
- **Performance Batch #3**: Added deferred + indexed message search in `useChatManager` to reduce repeated per-message lowercasing and smooth search updates while typing.
- **Performance Batch #4**: Optimized stream update paths by replacing callback state-read hacks with local message snapshots in tool loops, throttling multi-model streaming fanout, reducing nested chat/message mapping on streaming ticks, coalescing in-flight DB stream saves, and deferring non-critical startup services until first paint.
- **Performance Batch #5**: Added virtualization for project list mode and confirmed memories to reduce renderer work for large datasets.
- **Performance Batch #6**: Switched chat startup hydration to metadata-first loading and lazy message fetch per selected chat to avoid full message payload loading at app start.
- **Performance Batch #7**: Added serialized background PDF export queueing and converted data migration flows from sync filesystem calls to async chunked operations.
- **Performance Batch #8**: Added repository-level paginated SELECT helper and applied it to high-volume chat/project/knowledge read paths to avoid unbounded in-memory scans.
- **Performance Batch #9**: Finished remaining renderer PERF items with sidebar chat list virtualization, cached think/plan section parsing, memoized markdown output for stable messages, lazy-loaded markdown renderer module, and split root app callbacks/subscriptions to reduce avoidable tree rerenders.
...
- **DEBT-07**: Standardized error handling across Main and Shared processes by consolidating `TengraError`, `ApiError`, and `NetworkError` into a single shared utility.
...
- **Type**: fix
- **Status**: completed
- **Summary**: Resolved a race condition between the `tengra-token-service` (Node/Rust) and embedded Go Proxy that caused Codex (OpenAI) token reuse errors.
...
1. **Service Compatibility & Bridging**:
    - Tightened the contract between TypeScript `DatabaseService` and Rust `tengra-db-service`.
...
### Key Achievements
1. **Rust Database Service (`tengra-db-service`)**:
...
2. **Windows Service Integration**:
    ...
    - Service discovery via port file (`%APPDATA%/Tengra/services/db-service.port`)
...
## 2026-01-25: 🤖 Tengra PROJECT AGENT - AUTONOMOUS DEVELOPER
**Status**: ✅ COMPLETED
**Summary**:
Implemented the **Tengra Project Agent**, a fully autonomous AI developer capable of executing complex, multi-step coding tasks directly within the IDE. The agent operates in a "Think -> Plan -> Act -> Observe" loop, maintains context across sessions, and includes built-in resilience for API limits.
**Key Achievements**:
...
- **System Integration**:
    ...
    - Full integration with Tengra's Tool Executor (run commands, edit files, etc.).
- **Type Safety**:
...
**Result**: Tengra now meets enterprise standards for testing, security, and code quality! 🚀
## Recent Updates
...
**Status**: ✅ Completed
**Features**:
- **Simplified Theme System**: Restricted application themes to a clean "Tengra White" (Light) and "Tengra Black" (Dark) model, enforcing consistency.
...
Tengra has been completely transformed into an enterprise-ready application with dramatic performance improvements, comprehensive security hardening, enhanced architecture, and perfect type safety. The application now handles enterprise workloads (10,000+ items) with optimal resource usage.
...
**Result**: Tengra is now **enterprise-grade performant** and ready for heavy production workloads with thousands of chats, projects, and messages.
...
**Next-Generation Foundation**: Tengra is now built on enterprise-grade foundations ready for### [2026-01-26]
...
- **Bidirectional Persistence** ✅:
    ...
    - Updated Go proxy's `HTTPAuthStore.Save` to push refreshed tokens back to Tengra's database immediately upon refresh.
