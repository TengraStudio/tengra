...
### エージェント・アーキテクチャの強化：モノリシック・サービスのリファクタリングとカウンシル・オーケストレーション

- **Type**: refactor
- **Status**: completed
- **Summary**: モノリシックな AgentCollaborationService を 6 つのドメイン固有サービスに分解し、プランの準備とクォータ対応のルーティングを強化するために CouncilService を実装しました。

- **分解 (AI-SYS-14)**: 投票、討論、メッセージング、チームワーク、コンセンサス、ルーティングを、単一責任のスタンドアロン・サービスに抽出しました。
- **CouncilService (MARCH1-COUNCIL-001)**: クォータ対応のモデル割り当てと、優先度の高いステップの明示的な承認を備えた高度なプラン・オーケストレーションを実装しました。
- **AgentTaskExecutor (AI-SYS-13)**: プラン拡充のために CouncilService を統合し、水平スケーラビリティのギャップに対応しました。
- **型安全性**: エージェント・ドメイン全体で 40 以上の構造的な型エラーを解決し、100 以上の安全でないキャストを排除しました。
- **信頼性**: ヘルパー・エージェントのハンドオフのためのヒューリスティックなマージ・ゲート評価と、仲裁ベースのコンセンサス構築を実装しました。

### グローバルチャットエラーハンドリングと失敗時のUX

- **Type**: refactor
- **Status**: completed
- **Summary**: アプリケーション全体でチャットエラータイプを統一し、AIアシスタントのサイドバーにカテゴリー分類と再試行機能を備えた一貫した失敗時のUXを実装しました。

- **共有エラータイプ**: `ChatError`と`ChatErrorKind`をグローバル共有タイプとして定義し、メインプロセスとレンダラープロセス間の一貫性を確保しました。
- **エラーのカテゴリー分類**: `categorizeError`ユーティリティを実装し、ストリームエラーを`quota_exhausted`、`provider_unavailable`、`timeout`、または`generic`の種類に分類するようにしました。
- **失敗時のUX (MARCH1-CORE-001)**: `ChatErrorBanner`をワークスペースのAIアシスタントサイドバーに統合し、条件付きレンダリングとコンテキストに応じた再試行アクションを追加しました。
