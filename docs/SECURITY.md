...
Security is built into every layer of Tandem. We focus on protecting user credentials, securing local data storage, and hardening the communication between our various processes.

## Token Management and Synchronization

Tandem utilizes a stateless proxy architecture. This design ensures that sensitive credentials are never stored in plain text on the filesystem and are only held in memory or within an encrypted database.
...
- **On-Demand Pull**: The Go proxy retrieves decrypted tokens from Tandem's internal `AuthAPIService` only when needed for an outgoing request. This request is authenticated using a system-generated secret key.
