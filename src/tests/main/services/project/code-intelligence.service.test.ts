...
vi.mock('@main/services/project/code-intelligence/symbol-parser.util', () => ({
    parseFileSymbols: vi.fn(() => [])
}));

vi.mock('@main/services/project/code-intelligence/file-scanner.util', () => ({
...
vi.mock('@main/services/project/code-intelligence/symbol-navigation.util', () => ({
...
vi.mock('@main/services/project/code-intelligence/documentation-generator.util', () => ({
...
vi.mock('@main/services/project/code-intelligence/rename-symbol.util', () => ({
...
vi.mock('@main/services/project/code-intelligence/code-quality-scanner.util', () => ({
...
import { analyzeCodeQuality } from '@main/services/project/code-intelligence/code-quality-scanner.util';
import {
    generateFileDocumentation,
    generateProjectDocumentation,
    getFileOutline} from '@main/services/project/code-intelligence/documentation-generator.util';
import { scanDirForTodos,scanDirRecursively } from '@main/services/project/code-intelligence/file-scanner.util';
import { renameSymbol } from '@main/services/project/code-intelligence/rename-symbol.util';
import {
    findDefinition,
    findImplementations,
    findSymbols,
    ...
    searchFiles} from '@main/services/project/code-intelligence/symbol-navigation.util';
import { parseFileSymbols } from '@main/services/project/code-intelligence/symbol-parser.util';
import type { CodeSymbol } from '@main/services/project/code-intelligence/types';
...
function createMockDb(): {
    ...
    return {
        ...
        getCodeSymbolsByProjectPath: vi.fn(async () => []),
...
describe('CodeIntelligenceService', () => {
    ...
    describe('getSymbolAnalytics', () => {
        it('should aggregate symbol statistics correctly', async () => {
            mockDb.getCodeSymbolsByProjectPath.mockResolvedValue([
        ...
        it('should return fallback analytics on database error', async () => {
            mockDb.getCodeSymbolsByProjectPath.mockRejectedValue(new Error('DB failure'));
            const analytics = await service.getSymbolAnalytics('/root');
            expect(analytics.totalSymbols).toBe(0);
            expect(analytics.uniqueFiles).toBe(0);
    ...
    describe('indexProject', () => {
        it('should index all scanned files and mark project as indexed', async () => {
            ...
            await service.indexProject('/root', 'proj-1');
        ...
        it('should skip indexing when already in progress', async () => {
            ...
            const promise1 = service.indexProject('/root', 'proj-dup');
            // Yield to allow promise1 to start and register as in-progress
            await new Promise(r => setTimeout(r, 5));

            const promise2 = service.indexProject('/root', 'proj-dup');
        ...
        it('should skip indexing when project is already indexed (no force)', async () => {
            ...
            await service.indexProject('/root', 'proj-cached');
            vi.clearAllMocks();

            await service.indexProject('/root', 'proj-cached', false);
        ...
        it('should re-index when force=true even if already indexed', async () => {
            ...
            await service.indexProject('/root', 'proj-force');
            vi.clearAllMocks();
        ...
        it('should send progress events to browser windows', async () => {
            ...
            await service.indexProject('/root', 'proj-progress');
        ...
        it('should handle indexing failure gracefully', async () => {
            ...
            await service.indexProject('/root', 'proj-fail');
        ...
        it('should store symbols with embeddings during indexing', async () => {
            ...
            await service.indexProject('/root', 'proj-symbols');
    ...
    describe('scanProjectTodos', () => {
        it('should group todos by file with relative paths', async () => {
            ...
            const results = await service.scanProjectTodos('/root');
    ...
    describe('generateProjectDocumentation', () => {
        it('should delegate to documentation-generator util', async () => {
            const result = await service.generateProjectDocumentation('/root', 20);
            expect(generateProjectDocumentation).toHaveBeenCalledWith('/root', 20);
            expect(result.success).toBe(true);
        });

        it('should default maxFiles to 30', async () => {
            await service.generateProjectDocumentation('/root');
