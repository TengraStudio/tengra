...
export function registerFileDiffIpc(
    ...
    // Get file history
    ipcMain.handle('diff:getFileHistory', async (_event, filePath: string, limit?: number) => {
        try {
            const history = await fileChangeTracker.databaseService.getFileDiffHistory(filePath, limit)
    ...
    ipcMain.handle('diff:getChangesBySystem', async (_event, aiSystem: string, limit?: number) => {
        try {
            const changes = await fileChangeTracker.databaseService.getFileDiffsBySystem(aiSystem, limit)
            return { success: true, data: changes }
        } catch (error) {
            return { success: false, error: (error as Error).message }
        }
