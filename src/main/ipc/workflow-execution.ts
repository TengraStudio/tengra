...
import { registerWorkflowCanvasHandlers } from './workflow-canvas';
import { registerWorkflowCouncilHandlers } from './workflow-council';
import { registerWorkflowDecisionHandlers } from './workflow-decision';
...
type TaskMessagesResult = Awaited<ReturnType<WorkflowExecutionService['getTaskMessages']>>;
...
function registerWorkflowExecutionAdvancedHandlers(
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; models: AvailableModelInfo[] }, []>(
            ...
            async (event): Promise<{ success: boolean; models: AvailableModelInfo[] }> => {
                ...
                const models = await workflowExecutionService.getAvailableModels();
                return {
                    success: true,
                    models
                };
        ...
        createValidatedIpcHandler<void, [number | { index: number; taskId?: string }]>(
            ...
            async (event, payload: number | { index: number; taskId?: string }): Promise<void> => {
                ...
                if (typeof payload === 'number') {
                    await workflowExecutionService.retryStep(payload);
                    return;
                }
                await workflowExecutionService.retryStep(payload.index, payload.taskId);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; error?: string }, [{ taskId: string; provider: string; model: string }]>(
            ...
            async (event, payload: { taskId: string; provider: string; model: string }): Promise<{ success: boolean; error?: string }> => {
                validateSender(event);
                const success = await workflowExecutionService.selectModel(
                    payload.taskId,
                    payload.provider,
                    payload.model
                );
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [{ taskId: string; stepId: string }]>(
            ...
            async (event, payload: { taskId: string; stepId: string }): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.approveStep(payload.taskId, payload.stepId);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [{ taskId: string; stepId: string }]>(
            ...
            async (event, payload: { taskId: string; stepId: string }): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.skipStep(payload.taskId, payload.stepId);
            },
            {
                argsSchema: z.tuple([z.object({ taskId: z.string().min(1), stepId: z.string().min(1) })]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [{ taskId: string; stepId: string; text: string }]>(
            ...
            async (event, payload: { taskId: string; stepId: string; text: string }): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.editStep(payload.taskId, payload.stepId, payload.text);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [{ taskId: string; stepId: string; comment: string }]>(
            ...
            async (event, payload: { taskId: string; stepId: string; comment: string }): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.addStepComment(payload.taskId, payload.stepId, payload.comment);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [{ taskId: string; afterStepId: string }]>(
            ...
            async (event, payload: { taskId: string; afterStepId: string }): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.insertInterventionPoint(payload.taskId, payload.afterStepId);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [string]>(
            ...
            async (event, checkpointId: string): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.resumeFromCheckpoint(checkpointId);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<AgentCheckpointItem[], [string]>(
            ...
            async (event, taskId: string): Promise<AgentCheckpointItem[]> => {
                validateSender(event);
                return await workflowExecutionService.getCheckpoints(taskId);
            },
            {
                argsSchema: z.tuple([z.string().min(1)]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<RollbackCheckpointResult | null, [string]>(
            ...
            async (event, checkpointId: string): Promise<RollbackCheckpointResult | null> => {
                validateSender(event);
                return await workflowExecutionService.rollbackCheckpoint(checkpointId);
            },
            {
                argsSchema: z.tuple([z.string().min(1)]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<PlanVersionItem[], [string]>(
            ...
            async (event, taskId: string): Promise<PlanVersionItem[]> => {
                validateSender(event);
                return await workflowExecutionService.getPlanVersions(taskId);
            },
            {
                argsSchema: z.tuple([z.string().min(1)]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<boolean, [string]>(
            ...
            async (event, nodeId: string): Promise<boolean> => {
                validateSender(event);
                return await workflowExecutionService.deleteTaskByNodeId(nodeId);
            },
            {
                argsSchema: z.tuple([z.string().min(1)]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; url?: string; error?: string } | null, [{ taskId?: string } | undefined]>(
            ...
            async (event, payload?: { taskId?: string }): Promise<{ success: boolean; url?: string; error?: string } | null> => {
                validateSender(event);
                return await workflowExecutionService.createPullRequest(payload?.taskId);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<AgentProfile[], []>(
            ...
            async (event): Promise<AgentProfile[]> => {
                validateSender(event);
                return await workflowExecutionService.getProfiles();
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<AgentProfile | null, [AgentProfile]>(
            ...
            async (event, profile: AgentProfile): Promise<AgentProfile | null> => {
                validateSender(event);
                return await workflowExecutionService.registerProfile(profile);
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<boolean, [string]>(
            ...
            async (event, id: string): Promise<boolean> => {
                validateSender(event);
                return await workflowExecutionService.deleteProfile(id);
            },
            {
                argsSchema: z.tuple([z.string().min(1)]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<ModelRoutingRule[], []>(
            ...
            async (event): Promise<ModelRoutingRule[]> => {
                validateSender(event);
                return workflowExecutionService.getRoutingRules();
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: true }, [ModelRoutingRule[]]>(
            ...
            async (event, rules: ModelRoutingRule[]): Promise<{ success: true }> => {
                validateSender(event);
                workflowExecutionService.setRoutingRules(rules);
                return { success: true };
            },
            {
...
/**
 * Registers all workflow execution IPC handlers including core operations,
 * human-in-the-loop workflows, voting sessions, legacy compatibility,
 * and canvas persistence.
 * @param workflowExecutionService - The workflow service instance
 ...
 */
export function registerWorkflowExecutionIpc(
    workflowExecutionService: WorkflowExecutionService,
    ...
    const emitAgentEvent = (
        ...
        const currentTaskId = workflowExecutionService.getCurrentTaskId() ?? '';
        updateSequence += 1;
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ taskId: string }, [AgentStartOptions]>(
            ...
            async (event, options: AgentStartOptions): Promise<{ taskId: string }> => {
                validateSender(event);
                const taskId = await workflowExecutionService.start(options);
                return { taskId };
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [{ taskId?: string } | undefined]>(
            ...
            async (event, payload?: { taskId?: string }): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.stop(payload?.taskId);
            },
            {
                argsSchema: z.tuple([z.object({ taskId: z.string().optional() }).optional()]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: true }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: true }> => {
                validateSender(event);
                await workflowExecutionService.pauseTask(payload.taskId);
                return { success: true };
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; error?: string }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; error?: string }> => {
                validateSender(event);
                const success = await workflowExecutionService.resumeTask(payload.taskId);
                return {
                    success,
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; checkpointId: string }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; checkpointId: string }> => {
                validateSender(event);
                const checkpointId = await workflowExecutionService.saveSnapshot(payload.taskId);
                return {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, []>(
            ...
            async (event): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.resetState();
            },
            {
                wrapResponse: true
            }
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<void, [AgentStartOptions]>(
            ...
            async (event, options: AgentStartOptions): Promise<void> => {
                validateSender(event);
                await workflowExecutionService.generatePlan(options);
            },
            {
                argsSchema: z.tuple([AgentStartOptionsSchema]),
                wrapResponse: true
        ...
        createValidatedIpcHandler<void, [WorkspaceStep[] | { plan: WorkspaceStep[]; taskId?: string }]>(
            ...
            async (
                ...
                if (Array.isArray(payload)) {
                    await workflowExecutionService.approvePlan(payload);
                    return;
                }
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; error?: string }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; error?: string }> => {
                validateSender(event);
                const success = await workflowExecutionService.approveCurrentPlan(payload.taskId);
                return {
                    success,
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; error?: string }, [{ taskId: string; reason?: string }]>(
            ...
            async (event, payload: { taskId: string; reason?: string }): Promise<{ success: boolean; error?: string }> => {
                validateSender(event);
                const success = await workflowExecutionService.rejectCurrentPlan(payload.taskId, payload.reason);
                return {
                    success,
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<TaskMessagesResult, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<TaskMessagesResult> => {
                validateSender(event);
                return await workflowExecutionService.getTaskMessages(payload.taskId);
            },
            {
                argsSchema: z.tuple([z.object({ taskId: z.string().min(1) })]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; events: AgentEventRecord[] }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; events: AgentEventRecord[] }> => {
                validateSender(event);
                return await workflowExecutionService.getTaskEvents(payload.taskId);
            },
            {
                argsSchema: z.tuple([z.object({ taskId: z.string().min(1) })]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: true }, [{ taskId: string; task: string }]>(
            ...
            async (event, payload: { taskId: string; task: string }): Promise<{ success: true }> => {
                validateSender(event);
                await workflowExecutionService.generatePlan({
                    task: payload.task,
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; events: AgentEventRecord[] }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; events: AgentEventRecord[] }> => {
                validateSender(event);
                const events = await workflowExecutionService.getTaskEvents(payload.taskId);
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; telemetry: TaskMetrics[] }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; telemetry: TaskMetrics[] }> => {
                validateSender(event);
                return await workflowExecutionService.getTaskTelemetry(payload.taskId);
            },
            {
                argsSchema: z.tuple([z.object({ taskId: z.string().min(1) })]),
                wrapResponse: true
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<AgentTaskHistoryItem[], [{ workspaceId?: string } | undefined]>(
            ...
            async (event, payload?: { workspaceId?: string }): Promise<AgentTaskHistoryItem[]> => {
                validateSender(event);
                return await workflowExecutionService.getTaskHistory(payload?.workspaceId ?? '');
            },
            {
    ...
    ipcMain.handle(
        ...
        createValidatedIpcHandler<{ success: boolean; error?: string }, [{ taskId: string }]>(
            ...
            async (event, payload: { taskId: string }): Promise<{ success: boolean; error?: string }> => {
                validateSender(event);
                const success = await workflowExecutionService.deleteTask(payload.taskId);
                return {
                    success,
    ...
    registerWorkflowExecutionAdvancedHandlers(workflowExecutionService, validateSender);

    registerWorkflowDecisionHandlers(workflowExecutionService, getMainWindow);

    registerWorkflowCouncilHandlers(workflowExecutionService, getMainWindow);
    ...
    registerWorkflowCanvasHandlers(getMainWindow, databaseService, workflowExecutionService);
}
