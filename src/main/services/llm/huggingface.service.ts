...
export class HuggingFaceService extends BaseService {
    ...
    async searchModels(
        ...
        const cacheKey = `${query}:${limit}:${page}:${sort}`;
        ...
        try {
            ...
            const models = data.map((m) => {
                const model: HFModel = {
                    ...
                    description: m.cardData?.short_description ?? `A quality model by ${m.author || 'unknown'}`,
    ...
    async getRecommendations(limit: number = 10, query: string = ''): Promise<HFModel[]> {
        const { models } = await this.searchModels(query, Math.max(40, limit * 3), 0, 'downloads');
        return [...models]
    ...
    async validateModelCompatibility(
        ...
        if (file.quantization === 'UNKNOWN') {
            reasons.push('Quantization unknown; compatibility estimate is conservative');
        }
    ...
    getOptimizationSuggestions(options: HFConversionOptions): string[] {
        ...
        if (options.quantization === 'Q4_K_M') {
            suggestions.push('Q4_K_M is recommended for local CPU inference.');
        }
        if (options.quantization === 'Q6_K' || options.quantization === 'F16') {
    ...
    async convertModelToGGUF(
        ...
        onProgress?.({ stage: 'validate', percent: 5, message: 'Validation completed' });

        const fs = await import('fs/promises');
        const fsSync = await import('fs');
        const sourceLower = options.sourcePath.toLowerCase();

        try {
            if (sourceLower.endsWith('.gguf')) {
                onProgress?.({ stage: 'convert', percent: 20, message: 'Source is GGUF, preparing copy' });
                await fs.copyFile(options.sourcePath, options.outputPath);
                onProgress?.({ stage: 'finalize', percent: 100, message: 'Conversion completed via GGUF copy' });
                if (options.quantization !== 'F16') {
                    warnings.push('Quantization step skipped (no quantizer backend found); output copied as-is.');
                }
                return { success: true, warnings };
            }

            const python = await this.findPythonExecutable();
            const converterScript = await this.findConverterScript();
            if (!python || !converterScript) {
                return {
                    success: false,
                    error: 'Conversion backend not found. Install llama.cpp converter script or provide GGUF input.'
                };
            }

            onProgress?.({ stage: 'convert', percent: 25, message: 'Starting HF -> GGUF conversion' });
            const convertArgs = [
                converterScript,
                options.sourcePath,
                '--outfile',
                options.outputPath,
                '--outtype',
                options.quantization === 'F16' ? 'f16' : 'f16'
            ];

            const conversionResult = await this.runProcessWithProgress(
                python,
                convertArgs,
                (line) => {
                    const parsed = this.parsePercent(line);
                    onProgress?.({
                        stage: 'convert',
                        percent: parsed !== null ? Math.max(25, Math.min(80, parsed)) : 40,
                        message: line.slice(0, 180)
                    });
                }
            );
            ...
            if (options.quantization !== 'F16') {
                const quantizeBin = await this.findQuantizeBinary();
                if (!quantizeBin) {
                    warnings.push('Quantizer binary not found; model remains F16.');
                } else {
                    onProgress?.({ stage: 'quantize', percent: 82, message: `Applying quantization ${options.quantization}` });
                    const quantizedOutput = options.outputPath.replace(/\.gguf$/i, `.${options.quantization}.gguf`);
                    const quantResult = await this.runProcessWithProgress(
                        quantizeBin,
                        [options.outputPath, quantizedOutput, options.quantization],
                        (line) => {
                            const parsed = this.parsePercent(line);
                            onProgress?.({
                                stage: 'quantize',
                                percent: parsed !== null ? Math.max(82, Math.min(97, parsed)) : 90,
                                message: line.slice(0, 180)
                            });
                        }
                    );
                    if (quantResult.success) {
                        await fs.copyFile(quantizedOutput, options.outputPath);
                        await fs.unlink(quantizedOutput).catch(() => undefined);
                    } else {
                        warnings.push(`Quantization failed: ${quantResult.error}`);
                    }
                }
            }

            if (!fsSync.existsSync(options.outputPath)) {
                return { success: false, error: 'Conversion completed but output file is missing' };
            }

            onProgress?.({ stage: 'finalize', percent: 100, message: 'Conversion completed' });
            return { success: true, warnings };
        } catch (error) {
    ...
    private async runProcessWithProgress(
        command: string,
        args: string[],
        ...
        return new Promise((resolve) => {
            ...
            child.on('error', (error) => resolve({ success: false, error: getErrorMessage(error) }));
            child.on('close', (code) => {
                if (code === 0) {
                    resolve({ success: true });
                    return;
                }
                resolve({ success: false, error: stderr || `Process exited with code ${code ?? -1}` });
            });
        });
    }
    ...
    private async findPythonExecutable(): Promise<string | null> {
        ...
        for (const candidate of candidates) {
            const result = await this.runProcessWithProgress(candidate, ['--version'], () => undefined);
            if (result.success) {
                return candidate;
            }
        }
    ...
    private async findConverterScript(): Promise<string | null> {
        const fs = await import('fs/promises');
        const candidates = [
            path.join(process.cwd(), 'resources', 'llama.cpp', 'convert_hf_to_gguf.py'),
            path.join(process.cwd(), 'vendor', 'llama.cpp', 'convert_hf_to_gguf.py'),
            path.join(process.cwd(), 'scripts', 'convert_hf_to_gguf.py')
        ];
        for (const candidate of candidates) {
            try {
    ...
    private async findQuantizeBinary(): Promise<string | null> {
        const fs = await import('fs/promises');
        const candidates = [
            path.join(process.cwd(), 'resources', 'llama.cpp', process.platform === 'win32' ? 'llama-quantize.exe' : 'llama-quantize'),
            path.join(process.cwd(), 'vendor', 'llama.cpp', process.platform === 'win32' ? 'llama-quantize.exe' : 'llama-quantize')
        ];
        for (const candidate of candidates) {
            try {
                await fs.access(candidate);
