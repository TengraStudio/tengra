import { CouncilSession, DatabaseService } from '@main/services/data/database.service'
...
export class AgentCouncilService {
    ...
    async createSession(goal: string): Promise<CouncilSession> {
        return this.deps.db.createCouncilSession(goal)
    }
    ...
    async getSession(id: string): Promise<CouncilSession | null> {
        return this.deps.db.getCouncilSession(id)
    }
    ...
    async addLog(sessionId: string, agentId: string, message: string, type: 'info' | 'error' | 'success' | 'plan' | 'action' = 'info') {
        const log = await this.deps.db.addCouncilLog(sessionId, agentId, message, type)
    ...
    async runSessionStep(sessionId: string) {
        ...
        if (!session.plan || session.status === 'planning') {
            await this.handlePlanningStep(sessionId, session)
            return
        }
    ...
    private async handlePlanningStep(sessionId: string, session: CouncilSession) {
