...
describe('ChatEventService', () => {
    ...
    it('should append an event successfully', () => {
        ...
        const event = service.appendEvent(threadId, 'message_sent', payload);
    ...
    it('should get events for a thread', () => {
        const mockRow = {
            id: 'evt-1',
        ...
        const stmtMock = { all: vi.fn().mockReturnValue([mockRow]) };
        mockDb.prepare.mockReturnValue(stmtMock);

        const events = service.getEvents('thread-123');
    ...
    it('should rebuild thread state correctly', () => {
        ...
        const stmtMock = { all: vi.fn().mockReturnValue(mockRows) };
        mockDb.prepare.mockReturnValue(stmtMock);

        const state = service.rebuildThreadState('1');
