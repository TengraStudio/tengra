 ...
 * Creates directory structures and starter files for approved workspace ideas
 */
export class WorkspaceScaffoldService extends BaseService {
    constructor() {
        super('WorkspaceScaffoldService');
    }

    /**
     * Scaffold a workspace based on the idea category
     */
    async scaffoldWorkspace(idea: WorkspaceIdea, targetPath: string): Promise<void> {
        this.logInfo(`Scaffolding workspace: ${idea.title} at ${targetPath}`);
        ...
        try {
            ...
            this.logInfo(`Workspace scaffold created successfully at ${targetPath}`);
        } catch (error) {
            this.logError(`Failed to scaffold workspace: ${getErrorMessage(error as Error)}`);
            throw error;
        }
    }
     ...
     */
    private async createWebsiteScaffold(targetPath: string, idea: WorkspaceIdea): Promise<void> {
        // Create directories
...
`;
     ...
     */
    private async createMobileAppScaffold(targetPath: string, idea: WorkspaceIdea): Promise<void> {
...
`;
     ...
     */
    private async createGameScaffold(targetPath: string, idea: WorkspaceIdea): Promise<void> {
        await mkdir(path.join(targetPath, 'src'), { recursive: true });
...
<body>
     ...
     */
    private async createCliToolScaffold(targetPath: string, idea: WorkspaceIdea): Promise<void> {
        await mkdir(path.join(targetPath, 'src'), { recursive: true });
...
`;
     ...
     */
    private async createDesktopScaffold(targetPath: string, idea: WorkspaceIdea): Promise<void> {
        await mkdir(path.join(targetPath, 'src'), { recursive: true });
...
`;
     ...
     */
    private async createGenericScaffold(targetPath: string, idea: WorkspaceIdea): Promise<void> {
        await mkdir(path.join(targetPath, 'src'), { recursive: true });
...
`;
    ...
    /**
     * Generate README content for the workspace
     */
    generateReadme(idea: WorkspaceIdea): string {
...
${idea.explanation ?? 'A new workspace generated by Tengra AI.'}

## Value Proposition

${idea.valueProposition ?? 'This workspace aims to solve real-world problems in its target domain.'}
...
\`\`\`
${this.getWorkspaceStructure(idea.category)}
\`\`\`
...
*Generated by [Tengra AI](https://github.com/Tengra-ai)*
`;
    }

    /**
     * Get workspace structure description based on category
     */
    private getWorkspaceStructure(category: IdeaCategory): string {
