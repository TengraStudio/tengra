...
export interface WorkspaceAgentSessionDependencies {
    databaseService: DatabaseService;
    modelRegistryService?: ModelRegistryService;
...
export class WorkspaceAgentSessionService extends BaseService {
    ...
    constructor(deps: WorkspaceAgentSessionDependencies) {
        super('WorkspaceAgentSessionService');
        this.databaseService = deps.databaseService;
        this.modelRegistryService = deps.modelRegistryService;
        this.memoryContextService = new MemoryContextService(deps.advancedMemoryService);
    }

    private rememberSessionEvent(
        sessionId: string,
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.LIST_BY_WORKSPACE)
    async listWorkspaceSessionsIpc(workspaceId: string): Promise<RuntimeValue> {
        const workspace = await requireWorkspace(workspaceId, this.databaseService);
        ...
        } as WorkspaceAgentSessionListResponse);
    }

    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.CREATE)
    async createWorkspaceSessionIpc(payload: {
        workspaceId: string;
        title: string;
        modes?: WorkspaceAgentSessionModes;
        strategy?: WorkspaceAgentSession['strategy'];
        ...
        const workspace = await requireWorkspace(payload.workspaceId, this.databaseService);
                const now = Date.now();
        ...
        const nextPersistence: WorkspaceAgentSessionPersistence = {
            ...
            recentSessionIds: [chatId, ...currentPersistence.recentSessionIds].slice(0, 50),
            composerDraft: '',
            updatedAt: now,
        };

        await this.databaseService.updateWorkspace(payload.workspaceId, {
            metadata: mergeWorkspacePersistence(workspace.metadata, nextPersistence),
        });

        const chat = await requireChat(chatId, this.databaseService);
        return serializeToIpc(await toWorkspaceAgentSession(chat, this.databaseService, this.modelRegistryService));
    }

    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.RENAME)
    async renameWorkspaceSessionIpc(payload: { sessionId: string; title: string }): Promise<RuntimeValue> {
        const chat = await requireChat(payload.sessionId, this.databaseService);
        await this.databaseService.updateChat(chat.id, {
            title: payload.title,
            updatedAt: new Date(),
        });
        this.rememberSessionEvent(
        ...
        return serializeToIpc(await toWorkspaceAgentSession(
            await requireChat(chat.id, this.databaseService),
            this.databaseService,
            this.modelRegistryService
        ));
    }

    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.SELECT)
    async selectWorkspaceSessionIpc(payload: { workspaceId: string; sessionId: string | null }): Promise<RuntimeValue> {
        const workspace = await requireWorkspace(payload.workspaceId, this.databaseService);
        const currentPersistence = getWorkspacePersistence(workspace.metadata);
        const nextPersistence: WorkspaceAgentSessionPersistence = {
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.UPDATE_PERSISTENCE)
    async updateWorkspaceSessionPersistenceIpc(payload: {
        workspaceId: string;
        activeSessionId?: string | null;
        recentSessionIds?: string[];
        composerDraft?: string;
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.UPDATE_MODES)
    async updateWorkspaceSessionModesIpc(payload: {
        sessionId: string;
        modes: WorkspaceAgentSessionModes;
        status?: WorkspaceAgentSession['status'];
    }): Promise<RuntimeValue> {
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.UPDATE_PERMISSIONS)
    async updateWorkspaceSessionPermissionsIpc(payload: {
        sessionId: string;
        permissionPolicy: WorkspaceAgentPermissionPolicy;
    }): Promise<RuntimeValue> {
        const chat = await requireChat(payload.sessionId, this.databaseService);
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.UPDATE_STRATEGY)
    async updateWorkspaceSessionStrategyIpc(payload: {
        sessionId: string;
        strategy: WorkspaceAgentSession['strategy'];
    }): Promise<RuntimeValue> {
        const chat = await requireChat(payload.sessionId, this.databaseService);
    ...
    async archiveWorkspaceSessionIpc(payload: { sessionId: string; archived: boolean }): Promise<RuntimeValue> {
        const chat = await requireChat(payload.sessionId, this.databaseService);
        const sessionMetadata = getSessionMetadata(chat);
        await this.databaseService.updateChat(chat.id, {
            metadata: mergeSessionMetadata(chat, {
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.DELETE)
    async deleteWorkspaceSessionIpc(payload: { sessionId: string }): Promise<RuntimeValue> {
        const chat = await requireChat(payload.sessionId, this.databaseService);
        const result = await this.databaseService.deleteChat(chat.id);
        this.rememberSessionEvent(
            chat.id,
            chat.workspaceId,
    ...
    @ipc(WORKSPACE_AGENT_SESSION_CHANNELS.RESUME_BACKGROUND_STATE)
    async resumeWorkspaceSessionBackgroundStateIpc(payload: { workspaceId: string; activeSessionId: string | null }): Promise<RuntimeValue> {
        const workspace = await requireWorkspace(payload.workspaceId, this.databaseService);
        const currentPersistence = getWorkspacePersistence(workspace.metadata);
        const sessions = await listWorkspaceSessions(payload.workspaceId, this.databaseService, this.modelRegistryService);

        for (const session of sessions) {
            const chat = await requireChat(session.id, this.databaseService);
            const sessionMetadata = getSessionMetadata(chat);
