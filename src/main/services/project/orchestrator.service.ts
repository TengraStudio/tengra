...
export class MultiAgentOrchestratorService extends BaseService {
     ...
     */
    async orchestrate(task: string, _projectId?: string): Promise<void> {
        this.state = {
            status: 'planning',
            currentTask: task,
            plan: [],
    ...
    private async runPlanningPhase(): Promise<void> {
        ...
        try {
            ...
            if (result && Array.isArray(result.plan) && typeof result.assignments === 'object') {
                this.state.plan = (result.plan as ProjectStep[]).map((s) => ({ ...s, status: 'pending' }));
    ...
    async approvePlan(plan?: ProjectStep[]): Promise<void> {
