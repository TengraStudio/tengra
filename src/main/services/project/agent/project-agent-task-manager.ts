...
import { AgentTaskExecutor } from '@main/services/project/agent/agent-task-executor';
import { CouncilService } from '@main/services/project/agent/council.service';
import { ProjectAgentCollaborationManager } from '@main/services/project/agent/project-agent-collaboration-manager';
...
export interface ProjectAgentTaskManagerDependencies {
    ...
    collaborationManager: ProjectAgentCollaborationManager;
}

export class ProjectAgentTaskManager {
    ...
    private readonly collaborationManager: ProjectAgentCollaborationManager;
    ...
    constructor(deps: ProjectAgentTaskManagerDependencies) {
    ...
    private async restoreActiveTasks(): Promise<void> {
        try {
            ...
            appLogger.error('ProjectAgentTaskManager', 'Failed to restore active tasks', error as Error);
        }
    }
    ...
    private async scheduleExecutionStart(taskId: string, priority?: AgentStartOptions['priority']): Promise<boolean> {
        ...
        if (this.activeExecutionTaskIds.size >= this.maxConcurrentExecutionTasks) {
            ...
            appLogger.info('ProjectAgentTaskManager', `Queued task ${taskId} with priority ${normalizedPriority}`);
            return false;
        }
        this.activeExecutionTaskIds.add(taskId);
        return true;
    ...
    private async drainExecutionQueue(): Promise<void> {
        while (
            ...
            try {
                ...
                appLogger.error('ProjectAgentTaskManager', `Failed to start queued task ${nextTask.taskId}`, error as Error);
    ...
    async getTaskHistory(projectId: string): Promise<AgentTaskHistoryItem[]> {
        try {
            ...
            appLogger.error('ProjectAgentTaskManager', 'Failed to get task history', error as Error);
            return [];
        }
    }
    ...
    async getPerformanceMetrics(taskId: string) {
        return await this.agentPerformanceService.getMetrics(taskId) ?? null;
    }
