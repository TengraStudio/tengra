...
} from '@main/services/session/orchestration/session-evidence-store';
import {
    persistConversationAssistantMessage,
    recordConversationTokens,
} from '@main/services/session/orchestration/session-persistence-manager';
import { injectConversationSystemPrompt } from '@main/services/session/orchestration/session-prompt-builder';
import {
    extractConversationQuery,
    handleConversationRagContext,
    injectConversationContext,
} from '@main/services/session/orchestration/session-rag-orchestrator';
import {
...
} from '@main/services/session/orchestration/session-stream-evidence-handler';
import { safeSendConversationChunk } from '@main/services/session/orchestration/session-stream-ipc-handler';
import {
...
export class SessionConversationService extends BaseService {
    ...
    constructor(private deps: SessionConversationDependencies) {
        super('SessionConversationService');
        this.chatSessionRegistryService = deps.chatSessionRegistryService;
        this.memoryContextService = new MemoryContextService(deps.advancedMemoryService);
        this.brainService = deps.brainService;
    }

    @ipc({ channel: SESSION_CONVERSATION_CHANNELS.COMPLETE, argsSchema: sessionConversationCompleteRequestSchema, withEvent: true })
    async handleConversationComplete(event: IpcMainInvokeEvent, params: z.infer<typeof sessionConversationCompleteRequestSchema>): Promise<unknown> {
        const { messages, model, provider, tools, workspaceId, systemMode } = params;
        const chatId = params.workspaceId ?? 'system'; // Default if not provided
        ...
        if (sanitized.workspaceId && sanitized.messages.length > 0) {
            sources = await handleConversationRagContext(sanitized.messages, sanitized.workspaceId, this.deps.contextRetrievalService);
        }
        ...
        const finalMessages = injectConversationSystemPrompt({
            ...
            localeService: this.deps.localeService,
            permissionPolicy,
        ...
        try {
            ...
            return serializeToIpc(createConversationCompleteResult({
                messages: sanitized.messages,
                settingsService: this.deps.settingsService,
                systemMode: sanitized.systemMode,
                content: result.content,
    ...
    @ipc({ channel: SESSION_CONVERSATION_CHANNELS.STREAM, argsSchema: sessionConversationStreamRequestSchema, withEvent: true })
    async handleChatStream(event: IpcMainEvent | IpcMainInvokeEvent, params: z.infer<typeof sessionConversationStreamRequestSchema>): Promise<void> {
        const { messages, model, provider, tools, workspaceId, systemMode, chatId, assistantId, streamId, optionsJson } = params;
        const sanitized = sanitizeConversationStreamInputs({ messages, model, provider, tools, workspaceId, systemMode, chatId, assistantId, streamId, optionsJson });
        appLogger.info(
            'Chat',
    ...
    private async injectRAGContextForStream(params: {
        ...
        try {
            ...
            const { contextString, sources } = await this.deps.contextRetrievalService.retrieveContext(query, workspaceId);
    ...
    private async handleProxyStream(params: {
        ...
        try {
            ...
            for await (const chunk of this.deps.llmService.chatStream(messages, model, providedTools, provider, {
                systemMode,
     ...
     */
    @ipc({ channel: SESSION_CONVERSATION_CHANNELS.RETRY_WITH_MODEL, argsSchema: sessionConversationRetryRequestSchema, withEvent: true })
    async handleRetryWithModel(event: IpcMainInvokeEvent, params: z.infer<typeof sessionConversationRetryRequestSchema>): Promise<boolean> {
        const { chatId, messageId, model, provider } = params;

        const dbMessages = await this.deps.databaseService.chats.getMessages(chatId);
    ...
    private async getPermissionPolicy(chatId?: string): Promise<string | undefined> {
        ...
        const chat = await this.deps.databaseService.chats.getChat(chatId);
