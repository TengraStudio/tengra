...
pub async fn fetch_codex_quota(token: &str) -> Result<QuotaResult> {
    ...
    let quota = if let Some(rate_limit) = wham.get("rate_limit").or_else(|| wham.get("usage")) {
        let five_hour = pick_window(rate_limit, &["primary_window", "five_hour", "five_hour_window", "5h"]);
        let weekly = pick_window(rate_limit, &["secondary_window", "weekly", "weekly_window", "7d"]);
...
fn to_used_percent_window(window: &Value) -> Option<f64> {
    if let Some(value) = read_number(window, &["used_percent", "usage_percent", "percentage_used"]) {
...
fn extract_reset_at(window: &Value) -> Option<String> {
    if let Some(s) = read_value(window, &["reset_at", "resets_at", "resetAt"]).and_then(|v| v.as_str()) {
        return Some(s.to_string());
    }
    if let Some(n) = read_value(window, &["reset_at", "resets_at", "resetAt"]).and_then(|v| v.as_f64()) {
    ...
    if let Some(windows) = rate_limit.get("windows").and_then(|v| v.as_array()) {
        for key in keys {
            ...
            if let Some(found) = windows.iter().find(|item| {
                ...
                let kind = obj.and_then(|o| o.get("type")).and_then(|v| v.as_str()).unwrap_or("");
                let combined = format!("{} {}", name.to_ascii_lowercase(), kind.to_ascii_lowercase());
...
fn read_number(obj: &Value, keys: &[&str]) -> Option<f64> {
    read_value(obj, keys).and_then(|value| value.as_f64())
}
