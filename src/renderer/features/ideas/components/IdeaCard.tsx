...
export const IdeaCard = memo<IdeaCardProps>(({ idea, onClick }) => {
    ...
    const cardContent = (
        <React.Fragment>
            ...
            <div
                className={cn(
                    'w-full h-full text-left p-5 rounded-xl backface-hidden absolute inset-0 transition-all duration-500',
            ...
            <div
                className={cn(
                    'w-full h-full text-left p-5 rounded-xl backface-hidden absolute inset-0 rotate-y-180 bg-card/50 backdrop-blur-md overflow-hidden flex flex-col transition-all duration-500',
            ...
            >
                ...
                <div className="space-y-3 overflow-y-auto flex-1 text-sm text-muted-foreground">
                    <p>
                        <strong>{t('ideas.idea.impact')}:</strong> {t('ideas.idea.impactHigh')}
                    </p>
                    <p>
                        <strong>{t('ideas.idea.effort')}:</strong> {t('ideas.idea.effortMedium')}
                    </p>
                    {idea.valueProposition && (
                        <div>
                            <strong className="block text-foreground/80 mb-1">
    ...
    return (
        <div
            className={cn(
                ...
                isHighPotential &&
                    'p-[1px] rounded-xl bg-gradient-to-r from-primary via-accent to-primary/70'
            )}
        >
            <div
                className={cn(
                    'relative w-full h-full transition-all duration-700 ease-in-out transform-style-3d shadow-lg rounded-xl hover:shadow-xl',
                    isFlipped ? 'rotate-y-180' : '',
                    !isHighPotential &&
                        'border border-border/50 bg-muted/20 backdrop-blur-sm hover:bg-muted/40 hover:border-border transition-all duration-200',
