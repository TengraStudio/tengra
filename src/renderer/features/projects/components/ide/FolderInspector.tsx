            ...
            <div className="grid grid-cols-1 gap-1">
                {scriptKeys.slice(0, 5).map(name => (
                    <div key={name} className="flex items-center justify-between group p-1.5 hover:bg-accent/20 rounded transition-colors cursor-pointer">
                        ...
                        <span className="font-mono text-[10px] text-muted-foreground truncate max-w-[120px]" title={String(scripts[name])}>{String(scripts[name])}</span>
                    </div>
                ))}
                {scriptKeys.length > 5 && (
                    <div className="text-[10px] text-muted-foreground italic px-1 pt-1">
                        + {scriptKeys.length - 5} more scripts
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="pt-2 border-t border-border/50 mt-2">
            ...
            <div className="flex flex-wrap gap-1">
                {depKeys.slice(0, 8).map(dep => (
                    <span key={dep} className="px-1.5 py-0.5 rounded text-[10px] bg-accent/20 border border-border/50 text-muted-foreground">
                        {dep}
                    </span>
                ))}
            </div>
...
const FolderContentView = ({ data, folderPath, rootPath }: { data: DirectoryAnalysis; folderPath: string; rootPath: string }) => {
    ...
    const displayPath = relativePath === '/' ? 'Root' : relativePath;
