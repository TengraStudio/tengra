...
export const pauseTaskHandler = async (taskId: string): Promise<void> => {
    await window.electron.projectAgent.pauseTask(taskId);
};
...
export const stopTaskHandler = async (taskId: string): Promise<void> => {
    await window.electron.projectAgent.stop(taskId);
};
...
export const saveSnapshotHandler = async (taskId: string): Promise<boolean> => {
    const result = await window.electron.projectAgent.saveSnapshot(taskId);
...
export const resumeTaskHandler = async (taskId: string): Promise<IpcInvokeResult> => {
    return await window.electron.projectAgent.resumeTask(taskId);
};
...
export const approvePlanHandler = async (taskId: string): Promise<boolean> => {
    const result = await window.electron.projectAgent.approveCurrentPlan(taskId);
    if (!result.success) {
...
export const rejectPlanHandler = async (taskId: string, reason?: string): Promise<boolean> => {
    const result = await window.electron.projectAgent.rejectCurrentPlan(taskId, reason);
    if (!result.success) {
...
export const approveStepHandler = async (taskId: string, stepId: string): Promise<void> => {
    await window.electron.projectAgent.approveStep(taskId, stepId);
};
...
export const skipStepHandler = async (taskId: string, stepId: string): Promise<void> => {
    await window.electron.projectAgent.skipStep(taskId, stepId);
};
...
export const editStepHandler = async (
    ...
    await window.electron.projectAgent.editStep(taskId, stepId, text);
};
...
export const addStepCommentHandler = async (
    ...
    await window.electron.projectAgent.addStepComment(taskId, stepId, comment);
};
...
export const insertInterventionHandler = async (
    ...
    await window.electron.projectAgent.insertInterventionPoint(taskId, afterStepId);
};
...
export const fetchTaskDetailsHandler = async (
    ...
    try {
        const [status, eventsResponse] = await Promise.all([
            window.electron.projectAgent.getStatus(taskId),
            window.electron.projectAgent.getTaskEvents(taskId),
        ]);
