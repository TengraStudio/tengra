...
const C_MCPCARD_1 = "h-8 px-3 flex items-center gap-2 rounded-lg text-destructive/40 hover:text-destructive hover:bg-destructive/5 transition-colors text-xs font-semibold";
            ...
            <div className="flex flex-1 flex-col min-w-0 py-0.5">
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <div className="h-8 px-3 flex items-center gap-1.5 rounded-lg bg-primary/5 text-primary text-xs font-semibold">
                ...
                <div className="mt-1.5 flex items-center gap-2 text-xs font-medium text-muted-foreground/40">
                    ...
                    <span className="font-semibold tracking-tight text-[10px] bg-muted/40 px-1.5 py-0.5 rounded-md text-muted-foreground/40">V{plugin.version || '1.0.0'}</span>
                    <span className="opacity-20">•</span>
                    <span className="tracking-wider uppercase opacity-80 text-[10px]">{plugin.source}</span>
                </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5 overflow-hidden max-h-7">
                        {plugin.actions.slice(0, 3).map((action, index) => (
                            <div 
                                ...
                                className="px-2 py-0.5 rounded-md bg-muted/20 text-[10px] font-semibold tracking-tight text-muted-foreground/40 border border-border/5"
                            >
                                {action.name}
                            </div>
                        ))}
                        {plugin.actions.length > 3 && (
                            <div className="px-2 py-0.5 rounded-md bg-muted/10 text-[10px] font-semibold text-muted-foreground/30">
                                +{plugin.actions.length - 3}
                            </div>
                        )}
                    </div>
