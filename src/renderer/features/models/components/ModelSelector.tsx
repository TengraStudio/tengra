...
export function ModelSelector({ selectedProvider, selectedModel, onSelect, settings, groupedModels, quotas = null, codexUsage = null, onOpenChange, contextTokens = 0 }: ModelSelectorProps) {
    ...
    useEffect(() => {
        if (isOpen && containerRef.current) {
            ...
            if (rect.left + dropdownWidth > window.innerWidth - 16) {
                // Shift left so dropdown ends 16px from right edge
    ...
    const categories = useMemo(() => {
        ...
        const catsMap: Record<string, { id: string, name: string, icon: any, color: string, bg: string, providerId: string, models: Map<string, { id: string, label: string, disabled: boolean, provider: string, type?: string }> }> = {
        ...
        const brandsMapping: Array<{ key: keyof GroupedModels, catKey: string }> = [
            { key: 'ollama', catKey: 'ollama' },
            { key: 'copilot', catKey: 'copilot' },
            { key: 'openai', catKey: 'openai' },
            { key: 'anthropic', catKey: 'claude' },
            { key: 'gemini', catKey: 'gemini' },
            { key: 'antigravity', catKey: 'antigravity' },
            { key: 'custom', catKey: 'custom' }
        ]
        ...
        for (const mapping of brandsMapping) {
            ...
            for (const m of models) {
                ...
                catsMap[mapping.catKey].models.set(id, {
                    id: id,
                    label: label,
        ...
        return Object.values(catsMap).map(cat => ({
            ...
            models: Array.from(cat.models.values()).filter(m => {
                ...
                return matchesSearch && (!hidden.has(m.id) || m.id === selectedModel);
            }).sort((a, b) => a.label.localeCompare(b.label))
        })).filter(cat => cat.models.length > 0)
    }, [groupedModels, searchQuery, settings, selectedModel, quotas, codexUsage])
    ...
    return (
        <div className="relative" ref={containerRef}>
            <button
                ...
                className="flex items-center gap-2 bg-muted/20 hover:bg-muted/30 border border-border/50 hover:border-border/80 rounded-lg px-2 sm:px-3 py-1.5 transition-all outline-none min-w-[140px] justify-between group/sel"
            >
                <div className="flex items-center gap-2 overflow-hidden flex-1">
                    ...
                    <div className="flex flex-col items-start leading-none overflow-hidden flex-1">
                        <div className="flex items-center justify-between w-full pr-1">
                            <span className="text-xs text-zinc-500 font-bold uppercase tracking-wider truncate">
                                {currentCategory?.name || selectedProvider}
                            </span>
                            {contextTokens > 0 && (
                                <span className={cn(
                                    "text-[9px] font-bold px-1 rounded leading-none transition-colors",
                        ...
                        <div className="flex items-center gap-1.5 font-medium text-foreground truncate w-full mt-0.5">
                            ...
                            {currentModelInfo?.type === 'image' && (
                                <div className="flex items-center gap-1 bg-emerald-500/10 px-1 rounded py-0.5 border border-emerald-500/20 shadow-sm shrink-0">
                                    ...
                                    <span className="text-[8px] font-black text-emerald-400 uppercase tracking-tight">IMAGE</span>
                                </div>
                            )}
                        </div>
                        {/* Context Meter Bar (#58/78) */}
                ...
                <ChevronDown className={cn("w-3 h-3 text-zinc-500 transition-transform shrink-0 ml-1", isOpen && "rotate-180")} />
            </button>
            ...
            {isOpen && createPortal(
                <AnimatePresence>
                    <motion.div
                        ...
                        style={{
                            position: 'fixed',
                            top: dropUp ? dropdownPos.top - 508 : dropdownPos.top,
                            left: dropdownPos.left,
                            zIndex: 9999
                        }}
                        className="w-72 max-h-[500px] flex flex-col bg-[#0A0A0A] border border-white/10 rounded-xl shadow-2xl overflow-hidden"
                    >
                                        ...
                                        <div className="px-1">
                                            {category.models.map(model => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        // Use category's providerId for routing:
                                                        // - copilot category â†’ 'copilot' â†’ CopilotService
                                                        // - ollama category â†’ 'ollama' â†’ Local Ollama
                                                        // - all others â†’ their providerId â†’ LLMService via Cliproxy
                                                ...
                                                >
                                                    <span className="truncate flex-1 flex items-center gap-2">
                                                        {model.label}
                                                        {model.type === 'image' && <ImageIcon className="w-3 h-3 text-emerald-400" />}
                                                    </span>
                                                    {model.type === 'image' && (
                                                        <span className="text-[9px] font-bold text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded leading-none mr-1">
                                                            IMAGE
                                                        </span>
                                                    )}
                                                    {model.disabled && (
