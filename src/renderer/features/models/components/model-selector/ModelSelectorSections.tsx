    ...
    <div className="px-4 py-3.5 border-b border-border/40 flex flex-wrap items-center gap-6 bg-muted/5">
        ...
        {(showReasoningTab || showPermissionsTab) && (
            <div className="flex items-center gap-1 bg-background/50 backdrop-blur-sm rounded-xl p-1 border border-border/40 shadow-sm ml-auto">
                ...
                >
                    {t('frontend.modelSelector.tabs.models')}
                </button>
                {showReasoningTab && (
                    <button
                    ...
                    >
                        {t('frontend.modelSelector.tabs.reasoning')}
                    </button>
                )}
                {showPermissionsTab && (
                    <button
                    ...
                    >
                        {t('frontend.workspaceAgent.permissions.title')}
                    </button>
                )}
            </div>
        )}
...
function renderProviderQuota(categoryId: string, options: {
            ...
            badges: (
                <div className="flex items-center gap-1.5">
                    <span className="text-sm text-primary font-bold bg-primary/10 px-2 py-0.5 rounded-full border border-primary/20 leading-none">
                        {remaining}/{limit || 0} {t('frontend.modelSelector.creditsLeft')}
                    </span>
                    {rateLimit && (
                        <span className="text-sm text-warning font-bold bg-warning/10 px-2 py-0.5 rounded-full border border-warning/20 leading-none">
                            {t('frontend.statistics.rateLimit')} {rateLimit.remaining}/{rateLimit.limit}
                        </span>
                    )}
                </div>
            ),
            progress: (
                <div className="px-4 pb-3.5 bg-muted/5">
                    <div className="flex items-center justify-between typo-body font-bold text-muted-foreground/60 mb-1.5">
                        <span className="uppercase ">{t('frontend.statistics.usageStatus')}</span>
    ...
    if (categoryId === 'codex') {
        ...
        return {
            badges: (
                <div className="flex items-center gap-2">
                    {hasDaily && <CircularQuota value={100 - (usage.dailyUsedPercent ?? 0)} label={t('frontend.modelSelector.quota.day')} />}
                    {hasWeekly && <CircularQuota value={100 - (usage.weeklyUsedPercent ?? 0)} label={t('frontend.modelSelector.quota.week')} />}
                </div>
            )
        };
    }
    ...
    return (
        <div className="border-b border-border/30 last:border-b-0">
            <button
                ...
                aria-label={`${category.name} ${t('frontend.modelSelector.categoryLabelSuffix')}`}
            >
                <div className={cn(
    ...
    return (
        <>
            ...
            {showCuratedSections && recentModelItems.length > 0 && (
                <ModelSection
                    title={t('frontend.modelSelector.recentModels')}
            ...
            {showCuratedSections && deprecatedModels.length > 0 && (
                <ModelSection
                    title={t('frontend.modelSelector.deprecated')}
            ...
            {showCuratedSections && (
                <div className="px-4 py-2.5 typo-body font-bold text-muted-foreground/60 bg-muted/20 border-b border-border/30 uppercase ">
                    {t('frontend.modelSelector.allModels')}
                </div>
            )}
            ...
            {modeFilteredCategories.length === 0 ? (
                <div className="p-12 text-center text-muted-foreground/50">
                    ...
                    <p className="text-sm font-medium">{t('frontend.modelSelector.noModelsFound')}</p>
                </div>
            ) : shouldVirtualize ? (
                <div className="h-500">
                    <Virtuoso
