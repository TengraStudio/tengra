...
export function ProjectTechStack({ frameworks, t }: ProjectTechStackProps) {
    return (
        <div className="bg-card/40 rounded-2xl border border-border p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                {frameworks.length === 0 && <span className="text-xs text-muted-foreground italic">{t('projectDashboard.noFrameworks')}</span>}
            </div>
        </div>
    );
}

interface ProjectLanguageDistributionProps {
    languages: Record<string, number | unknown>
    stats: ProjectStats | null
    t: (key: string) => string
}

export function ProjectLanguageDistribution({ languages, stats, t }: ProjectLanguageDistributionProps) {
    return (
        <div className="bg-card/40 rounded-2xl border border-border p-5 space-y-4">
            <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
...
interface ProjectAnalysisTodosProps {
    todos: string[]
    t: (key: string) => string
}

export function ProjectAnalysisTodos({ todos, t }: ProjectAnalysisTodosProps) {
...
export const WorkspaceTechStack = ProjectTechStack;
export const WorkspaceLanguageDistribution = ProjectLanguageDistribution;
export const WorkspaceAnalysisTodos = ProjectAnalysisTodos;
