...
const EmptyStateView = ({ t }: { t: (key: string) => string }) => (
    <div className="h-full flex flex-col items-center justify-center text-muted-foreground p-4 text-center">
        ...
        <p className="text-sm">{t('frontend.inspector.selectFolder')}</p>
    </div>
);
...
const LoadingView = ({ t }: { t: (key: string) => string }) => (
    <div className="p-4 text-sm text-muted-foreground flex items-center gap-2">
        ...
        {t('frontend.inspector.analyzing')}
    </div>
);
        ...
        <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-lg bg-accent/20 border border-border/50">
                <div className="text-sm text-muted-foreground mb-1">
                    {t('frontend.workspaceDashboard.folderInspector.files')}
                </div>
                <div className="text-xl font-bold">{fileCount}</div>
            </div>
            <div className="p-3 rounded-lg bg-accent/20 border border-border/50">
                <div className="text-sm text-muted-foreground mb-1">
                    {t('frontend.workspaceDashboard.folderInspector.size')}
                </div>
    ...
    return (
        <div className="pt-2 border-t border-border/50 mt-2">
            <div className="text-sm text-muted-foreground mb-2">
                {t('frontend.workspaceDashboard.folderInspector.scripts')}
            </div>
            <div className="grid grid-cols-1 gap-1">
                ...
                {scriptKeys.length > 5 && (
                    <div className="text-sm text-muted-foreground px-1 pt-1">
                        {t('frontend.workspaceDashboard.folderInspector.moreScripts', {
...
const DependenciesSection = ({ dependencies }: { dependencies: DependenciesRecord }) => {
    ...
    return (
        <div className="pt-2 border-t border-border/50 mt-2">
            <div className="text-sm text-muted-foreground mb-2">
                {t('frontend.workspaceDashboard.folderInspector.dependencies')}
            </div>
    ...
    return (
        <div className="space-y-3">
            <h3 className="typo-caption font-bold text-muted-foreground flex items-center gap-2">
                ...
                {t('frontend.workspaceDashboard.folderInspector.packageTitle')}
            </h3>
            <div className="bg-muted/30 rounded-lg p-3 border border-border/50 space-y-2">
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">
                        {t('frontend.workspaceDashboard.folderInspector.packageName')}
                    </span>
                    <span className="font-mono text-success">{String(pkg.name)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">
                        {t('frontend.workspaceDashboard.folderInspector.packageVersion')}
                    </span>
...
const FolderContentView = ({
    ...
    const displayPath =
        relativePath === '/' ? t('frontend.workspaceDashboard.folderInspector.root') : relativePath;
