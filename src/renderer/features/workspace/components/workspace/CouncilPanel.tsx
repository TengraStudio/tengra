...
const getLogDotColor = (log: ActivityEntry): string => {
    if (log.type === 'error') {
        return LOG_DOT_COLORS.error;
    }
    if (log.type === 'success') {
        return LOG_DOT_COLORS.success;
    }
    if (log.type === 'plan') {
        return LOG_DOT_COLORS.plan;
    }
    if (log.agentId === 'reviewer') {
        return 'bg-purple';
    }
...
const getAgentTitleColor = (log: ActivityEntry): string => {
    ...
    if (agentId === 'planner' || title === 'PLANNER') {
        return AGENT_TITLE_COLORS.planner;
    }
    if (agentId === 'executor' || title === 'EXECUTOR') {
        return AGENT_TITLE_COLORS.executor;
    }
    if (agentId === 'reviewer' || title === 'REVIEWER') {
        return AGENT_TITLE_COLORS.reviewer;
    }
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
            ...
            <div className="text-2xl font-bold text-foreground">
                {councilEnabled ? t('agents.enabled') : t('agents.disabled')}
            </div>
            <button
        ...
        <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
            ...
            <div className="text-xs text-muted-foreground">
                {localCount} {t('agents.local')} / {cloudCount} {t('agents.cloud')}
            </div>
            <button
                onClick={addAgent}
                className="w-full px-3 py-2 rounded-lg text-xs font-semibold border border-white/10 bg-white/5 text-muted-foreground hover:bg-white/10"
            >
                {t('agents.addAgent')}
            </button>
        </div>
        <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
            ...
            <textarea
                className="w-full h-16 bg-black/20 border border-white/10 rounded-lg p-2 text-xs text-muted-foreground resize-none focus:outline-none focus:border-primary/50 custom-scrollbar"
            ...
            <button
                ...
                className={cn(
                    'w-full px-3 py-2 rounded-lg text-xs font-semibold border border-white/10 bg-white/5 text-muted-foreground transition-colors',
                    councilEnabled && goal.trim()
                        ? 'hover:bg-white/10'
...
const AgentList: React.FC<AgentListProps> = ({ agents, enabledAgents, toggleAgent, t }) => (
    <div className="bg-card border border-border rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-xs text-muted-foreground">
                {enabledAgents.length} {t('agents.active')}
            </div>
        </div>
                ...
                >
                    <div
                        className={cn(
                            ...
                            agent.status === 'ready' ? 'bg-success' : 'bg-warning'
                        )}
                    />
    ...
    <div key={log.id} className="flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div className="space-y-1 flex-1 min-w-0">
            <div className="flex items-center gap-2">
                <span
                    className={cn(
                        'uppercase font-bold tracking-wider text-xxs',
                        getAgentTitleColor(log)
                    )}
                >
                    {log.agentId ?? log.title}
                </span>
            ...
            <div className="text-muted-foreground whitespace-pre-wrap leading-relaxed break-words">
                {log.message.split('```').map((part: string, i: number) =>
                    i % 2 === 1 ? (
                        <code
                            key={i}
                            className="block my-2 p-2 bg-white/5 rounded border border-white/5 text-warning-light/80"
                        >
                            {part.trim()}
                        </code>
                    ) : (
            ...
            <div className="bg-card border border-border rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="h-64 overflow-y-auto custom-scrollbar border border-white/10 rounded-xl bg-black/40 p-4 space-y-3 font-mono text-xs">
                    {activityLog.length === 0 ? (
                        <div className="text-neutral italic text-center py-10">
                ...
                <div className="bg-card border border-border rounded-2xl p-5">
                    ...
                    <div className="space-y-2 text-xs text-muted-foreground">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-muted-foreground">{t('agents.roundRobin')}</span>
                        </div>
                        <div className="flex items-center justify-between">
                    ...
                    <div className="space-y-2 text-xs text-muted-foreground">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-muted-foreground">
                                {t('agents.escalateToUser')}
                            </span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span>{t('agents.lateSuggestions')}</span>
                            <span className="text-muted-foreground">{t('agents.allowed')}</span>
                        </div>
                    </div>
                </div>
            </div>
