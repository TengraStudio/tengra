        ...
        <div className="space-y-6">
            <div className="bg-card p-8 rounded-xl border border-border flex flex-col items-center text-center space-y-6">
                ...
                <div className="flex gap-3">
                    <button onClick={() => window.electron.openExternal('https://github.com/agnes0912491/Tandem')} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-muted/30 hover:bg-muted/50 text-xs font-bold text-muted-foreground hover:text-foreground border border-border/50"><ExternalLink className="w-3.5 h-3.5" /> {t('about.privacyPolicy')}</button>
                    <button onClick={() => window.electron.openExternal('https://github.com/agnes0912491/Tandem')} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-muted/30 hover:bg-muted/50 text-xs font-bold text-muted-foreground hover:text-foreground border border-border/50"><ExternalLink className="w-3.5 h-3.5" /> {t('about.github')}</button>
                </div>
                <div className="pt-6 border-t border-border/50 w-full"><p className="text-xxs text-muted-foreground/50 uppercase tracking-widest">{t('about.copyright')}</p></div>
            </div>
            <div className="bg-card p-6 rounded-xl border border-border">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 rounded-lg bg-muted/30 border border-border/50">
                        <div className="text-xxs text-muted-foreground uppercase tracking-wide">{t('advanced.platform')}</div>
                        <div className="text-sm font-bold text-foreground">{navigator.platform}</div>
                    </div>
                    <div className="p-3 rounded-lg bg-muted/30 border border-border/50">
                        <div className="text-xxs text-muted-foreground uppercase tracking-wide">{t('about.locale')}</div>
                        <div className="text-sm font-bold text-foreground">{navigator.language}</div>
                    </div>
                    <div className="p-3 rounded-lg bg-muted/30 border border-border/50">
                        <div className="text-xxs text-muted-foreground uppercase tracking-wide">{t('about.buildSync')}</div>
                        <button
                    ...
                    <div className="p-3 rounded-lg bg-muted/30 border border-border/50">
                        <div className="text-xxs text-muted-foreground uppercase tracking-wide">{t('about.sessionStart')}</div>
                        <div className="text-sm font-bold text-success">{new Date().toLocaleTimeString()}</div>
                    </div>
                    <div className="p-3 rounded-lg bg-muted/30 border border-border/50">
                        <div className="text-xxs text-muted-foreground uppercase tracking-wide">{t('about.buildVersion')}</div>
                        <div className="text-sm font-bold text-primary flex flex-col">
                            ...
                            <span className="text-xxxs opacity-70 font-mono mt-1">
                                {typeof __BUILD_TIME__ !== 'undefined' ? new Date(__BUILD_TIME__).toLocaleString() : t('about.notAvailable')}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-card p-6 rounded-xl border border-destructive/20">
                <div className="flex items-center justify-between">
                    <div><h3 className="text-sm font-bold text-destructive flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> {t('about.factoryReset')}</h3><p className="text-xs text-muted-foreground mt-1">{t('about.factoryResetDesc')}</p></div>
                    <button onClick={onReset} className="px-4 py-2 rounded-lg text-xs font-bold bg-destructive/10 text-destructive hover:bg-destructive/20 border border-destructive/20">{t('about.factoryReset')}</button>
                </div>
            </div>
        </div>
    );
