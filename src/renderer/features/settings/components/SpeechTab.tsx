...
function useSpeechDevices(t: (key: string) => string) {
    ...
    useEffect(() => {
        ...
        const loadDevices = async () => {
            try {
                await navigator.mediaDevices.getUserMedia({ audio: true }).catch((err: Error) => appLogger.warn('SpeechTab', 'Mic permission denied:', err));
                const d = await navigator.mediaDevices.enumerateDevices();
                setDevices(d);
            } catch (err) {
                appLogger.error('SpeechTab', 'Error enumerating devices:', err instanceof Error ? err : new Error(String(err)));
            }
        };
    ...
    <div className="space-y-4">
        <div>
            ...
            <SelectDropdown
                ...
                options={[{ value: 'default', label: t('speech.systemDefault') }, ...inputDevices]}
        ...
        <div>
            ...
            <SelectDropdown
                ...
                options={[{ value: 'default', label: t('speech.systemDefault') }, ...outputDevices]}
    ...
    return (
        <div className="bg-card p-6 rounded-2xl border border-border">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h3 className="text-sm font-bold text-foreground uppercase tracking-wider">{t('speech.title')}</h3>
                    <p className="text-xs text-muted-foreground">{t('speech.subtitle')}</p>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <VoiceSection settings={settings} voices={voices} updateSpeech={updateSpeech} t={t} />
                <DeviceSection settings={settings} inputDevices={inputDevices} outputDevices={outputDevices} updateSpeech={updateSpeech} t={t} />
                <div className="flex flex-col justify-between md:col-span-2 mt-4 space-y-4">
                    ...
                    <button onClick={handleTest} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-primary/10 text-primary text-xs font-bold uppercase tracking-widest transition-all border border-primary/20 hover:bg-primary/20">
                        <Volume2 className="w-4 h-4" /> {t('speech.test')}
                    </button>
                </div>
            </div>
        </div>
