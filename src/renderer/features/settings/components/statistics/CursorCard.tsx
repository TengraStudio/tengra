...
import { CursorQuota } from '@/types/quota';
...
export const CursorCard: React.FC<CursorCardProps> = ({ cursorQuota, locale = 'en-US' }) => {
    const { t } = useTranslation();
    if (!cursorQuota?.accounts || cursorQuota.accounts.length === 0) { return null; }

    return (
        <div className="space-y-4">
            {cursorQuota.accounts.map((acc, idx: number) => {
                        ...
                        <div className="flex items-center justify-between border-b border-border/10 bg-muted/5 px-4 py-3">
                            ...
                            />
                                <span className="truncate text-sm font-bold text-foreground">
                                    {acc.email ?? t('frontend.statistics.cursorAccount')}
                                </span>
                            </div>
                        ...
                        {!acc.error && (
                            <div className="divide-y divide-border/5">
                                {[
                                    ...
                                    acc.weekly && { id: 'weekly', name: t('frontend.statistics.weeklyStatus'), ...acc.weekly }
