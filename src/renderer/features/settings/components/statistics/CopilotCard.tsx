        ...
        <Card className="border-border/40 bg-card backdrop-blur-md overflow-hidden relative group">
            ...
            <CardContent className="space-y-6">
                {copilotQuota.accounts.map((acc, idx: number) => {
                    if (acc.error) {
                        return (
                            <div key={acc.accountId || idx} className={cn("space-y-4", idx > 0 && "pt-6 border-t border-border/50")}>
                                <div className="text-xs font-bold text-success truncate">{acc.email || 'Copilot Account'}</div>
                    ...
                    return (
                        <div key={acc.accountId || idx} className={cn("space-y-4", idx > 0 && "pt-6 border-t border-border/50")}>
                            <div className="text-xs font-black text-success/80 uppercase tracking-widest">{acc.email ?? 'Copilot Account'}</div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div className="flex items-center gap-4">
                                    <QuotaRing value={percent} color="hsl(var(--success))" size="sm" />
                                    <div>
