    ...
    return (
        <div className="space-y-6">
            <div className="bg-card p-6 rounded-xl border border-border space-y-4">
                <div><h3 className="text-lg font-bold text-white">{t('settings.personas')}</h3><p className="text-xs text-muted-foreground">Yanıtı şekillendiren kişiselleştirme profilleri.</p></div>
                <div className="grid grid-cols-1 gap-3">
                    <input type="text" placeholder="Persona adı" value={personaDraft.name} onChange={e => setPersonaDraft({ ...personaDraft, name: e.target.value })} className="w-full bg-muted/20 border border-border/50 rounded-lg px-3 py-2 text-sm text-primary" />
                    <input type="text" placeholder="Kısa açıklama" value={personaDraft.description} onChange={e => setPersonaDraft({ ...personaDraft, description: e.target.value })} className="w-full bg-muted/20 border border-border/50 rounded-lg px-3 py-2 text-sm text-primary" />
