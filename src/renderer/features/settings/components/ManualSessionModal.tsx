...
const InputSection: React.FC<InputSectionProps> = ({ sessionKey, setSessionKey, isSaving, success, error, t }) => (
...
const SaveButtonContent: React.FC<SaveButtonProps> = ({ isSaving, success, t }) => {
    ...
    if (success) { return <><Check className="w-4 h-4" />{t('auth.validatedAndSaved')}</>; }
    return <>{t('auth.saveSessionKey')}</>;
};
...
export const ManualSessionModal: React.FC<ManualSessionModalProps> = ({
    isOpen, onClose, onSave, email, accountId
}) => {
    ...
    const validateSessionKey = useCallback((key: string): string | null => {
        if (!key.trim()) { return t('auth.enterSessionKey'); }
        if (!key.startsWith('sk-ant-sid')) { return t('auth.invalidSessionFormat'); }
        return null;
    }, [t]);

    const handleSaveSuccess = useCallback(() => {
        setSuccess(true);
        setTimeout(() => { onClose(); setSuccess(false); setSessionKey(''); }, 1500);
    }, [onClose]);
