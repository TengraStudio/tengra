        ...
        <div className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border border-border/40 transition-colors hover:bg-muted/40 hover:border-border/60">
            ...
            <div className="flex items-center gap-2">
                ...
                {onCapture && (
                    ...
                    >
                        {isRecording ? t('frontend.shortcuts.pressKeys') : t('common.edit')}
                    </button>
                )}
                {onReset && (
                    <button
                        ...
                        title={t('frontend.shortcuts.resetToDefault')}
                    >
                        {t('common.reset')}
                    </button>
                )}
            ...
            >
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-border/50 bg-muted/10">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 id="shortcuts-modal-title" className="text-lg font-bold text-foreground m-0">{t('frontend.shortcuts.title')}</h2>
                    </div>
                    <button
                        ref={closeButtonRef}
                        onClick={onClose}
                ...
                <div className="flex flex-wrap items-center gap-3 px-6 py-3 border-b border-border/50 bg-background/50">
                    <div className="relative flex-1 min-w-200">
                        ...
                        <input
                            ...
                            placeholder={t('frontend.shortcuts.searchPlaceholder')}
                    ...
                    >
                        <IconDownload className="w-3.5 h-3.5 text-muted-foreground" /> {t('frontend.shortcuts.export')}
                    </button>
                    <button
                        onClick={() => importInputRef.current?.click()}
                        className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-foreground bg-background border border-border/50 rounded-md cursor-pointer transition-colors hover:bg-muted shadow-sm"
                    >
                        <IconUpload className="w-3.5 h-3.5 text-muted-foreground" /> {t('frontend.shortcuts.import')}
                    </button>
                    <button
                        onClick={handleResetAll}
                        className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-foreground bg-background border border-border/50 rounded-md cursor-pointer transition-colors hover:bg-muted shadow-sm"
                    >
                        <IconRotate className="w-3.5 h-3.5 text-muted-foreground" /> {t('frontend.shortcuts.resetAll')}
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-background">
                    <div className="space-y-3">
                        <h3 className="text-sm font-semibold text-primary/80 uppercase ">{t('frontend.shortcuts.general')}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <ShortcutItem label={t('frontend.shortcuts.closeModal')} keys={['Esc']} />
                        </div>
                    </div>

                    <div className="space-y-3">
                        <h3 className="text-sm font-semibold text-primary/80 uppercase ">{t('frontend.shortcuts.navigation')}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="space-y-3">
                        <h3 className="text-sm font-semibold text-primary/80 uppercase ">{t('frontend.shortcuts.chat')}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <ShortcutItem keys={['Enter']} label={t('frontend.shortcuts.sendMessage')} />
                            <ShortcutItem keys={['Shift', 'Enter']} label={t('frontend.shortcuts.newLine')} />
                        </div>
                    </div>
                </div>
