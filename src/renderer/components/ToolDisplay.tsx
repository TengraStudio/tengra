...
export function ToolDisplay({ toolCall, result, isExecuting }: ToolDisplayProps) {
    ...
    let statusText = 'Araç kullanılıyor...'
    if (isExecuting) {
        if (toolCall.name.includes('search')) statusText = 'İnternette aranıyor...'
        else if (toolCall.name.includes('file')) statusText = 'Dosyalar okunuyor...'
        else if (toolCall.name.includes('command')) statusText = 'Komut yürütülüyor...'
        else if (toolCall.name.includes('screenshot')) statusText = 'Ekran görüntüsü alınıyor...'
    } else {
        if (hasError) statusText = 'İşlem başarısız oldu'
        else statusText = 'Tamamlandı'
    }
        ...
        <div className="my-2 group">
            ...
            {userExpanded && (
                <div className="mt-2 ml-2 border-l-2 border-white/10 pl-3 py-1 space-y-3 animate-in slide-in-from-top-1 duration-200">
                    ...
                    {result && (
                        <div className="space-y-1">
                            <div className="text-sm uppercase tracking-wider text-muted-foreground font-semibold opacity-70">Çıktı</div>
