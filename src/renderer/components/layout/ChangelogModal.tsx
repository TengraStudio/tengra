...
// --- Types ---
export interface ChangelogLocaleContent {
    title: string;
    summary?: string;
    items: string[];
}
...
export type ChangelogIndexEntry = ChangelogItem;
...
// --- Component ---
export const ChangelogModal = memo(({ isOpen, onClose }: ChangelogModalProps) => {
    const { t, language } = useTranslation();
    const currentLanguage = typeof language === 'string' ? language : 'en';
    ...
    }, [isOpen, changelogEntries.length]);
        ...
        return changelogGroups
            .map(group => {
                const nextItems = group.items.filter(item => {
                    ...
                    const content = getLocaleContent(item, currentLanguage);
                    const haystack = [
                        content.title,
                        content.summary ?? '',
                        ...content.items,
    ...
    }, [changelogGroups, dateFrom, dateTo, currentLanguage, searchQuery, typeFilter]);
    ...
    }, [changelogGroups, currentLanguage, searchHistory, searchQuery]);

    const commitSearch = useCallback((forcedQuery?: string) => {
    ...
    }, [searchHistory, searchQuery, scheduleSearchStorageWrite]);
    ...
    const exportFilteredResults = useCallback(() => {
        const exportPayload = filteredGroups.flatMap(group => group.items.map(item => ({
            ...
            type: item.type,
            status: item.status,
            content: getLocaleContent(item, currentLanguage)
        })));
    ...
    }, [filteredGroups, currentLanguage]);
