...
function getButtonClassName(active: boolean, variant: string, className?: string): string {
    const baseClasses = "w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-200 border border-transparent";
    ...
    const glassClasses = variant === 'glass' && !active ? "hover:bg-background/40 hover:backdrop-blur-sm" : "";
...
export const SidebarItem: React.FC<SidebarItemProps> = ({
    ...
    <div className="group/item relative">
        <button
            {...props}
            onClick={onClick}
            className={getButtonClassName(active, variant, className)}
        >
