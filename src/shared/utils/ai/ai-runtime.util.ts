...
import {
    ...
    AiRuntimeSystemMode,
    AiToolLoopBudget,
} from '@shared/types/ai/ai-runtime';
import { Message, ToolCall } from '@shared/types/ai/chat';
...
export function classifyAiIntent(message: Message, systemMode: AiRuntimeSystemMode): AiIntentClassification {
    ...
    const isGreeting = /^(hi|hello|hey|selam|merhaba|nasılsın|nasilsin|günaydın|gunaydin|tünaydın|tunaydin|iyi akşamlar|iyi aksamlar|hey tengra|hi tengra|hello tengra|selam tengra|merhaba tengra)$/i.test(text);

    if (isAgent && !isGreeting) {
        return {
    ...
    return {
        ...
        requiresTooling: false,
        preferredMaxModelTurns: 999,
        preferredMaxToolTurns: 16, // Allow tools if the model decides to use them, even for simple prompts
    };
}
...
export function buildAiPresentationMetadata(context: AiPresentationContext): AiPresentationMetadata {
    const {
        content,
        reasoning,
        reasoningSummary: providedSummary,
        reasonings = [],
        toolCalls,
        toolResults,
        sources,
    ...
    const normalizedReasoning = providedSummary 
        ? providedSummary 
    ...
    const intent = getIntentValue(context.intent);
    const isAgentic = intent === 'agentic_workflow';
    ...
    // Special case for agentic workflows: keep in running_tools if tool calls are present
    // even if some content was produced, to avoid premature "answer ready" UI flicker
    if (isAgentic && toolCallCount > 0 && isStreaming) {
        stage = 'running_tools';
    }
