...
type RemoteManagement struct {
	...
	DisableControlPanel bool `yaml:"disable-control-panel" json:"disable-control-panel"`
	// PanelGitHubRepository overrides the GitHub repository used to fetch the management panel asset.
	// Accepts either a repository URL (https://github.com/org/repo) or an API releases endpoint.
	PanelGitHubRepository string `yaml:"panel-github-repository" json:"panel-github-repository"`
}
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Support loading from a nested "proxy" key in JSON files (e.g. settings.json)
	if strings.HasSuffix(strings.ToLower(configFile), ".json") {
		var wrapper map[string]yaml.Node
		if err := yaml.Unmarshal(data, &wrapper); err == nil {
			if proxyNode, ok := wrapper["proxy"]; ok {
				// Re-encode specific node to bytes so it can be unmarshaled into Config
				if buf, err := yaml.Marshal(&proxyNode); err == nil {
					data = buf
				}
			}
		}
