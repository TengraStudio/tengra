...
func (e *AntigravityExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for idx, baseURL := range baseURLs {
		...
		if httpResp.StatusCode == http.StatusUnauthorized {
			log.Debugf("antigravity executor: received 401, attempting token refresh")
			if refreshedAuth, errRefresh := e.refreshToken(ctx, auth); errRefresh == nil {
				auth = refreshedAuth
				token = metaStringValue(auth.Metadata, "access_token")
				// Re-build request with new token
				httpReq, errReq = e.buildRequest(ctx, auth, token, req.Model, translated, false, opts.Alt, baseURL)
				if errReq == nil {
					if httpRespRetry, errDoRetry := httpClient.Do(httpReq); errDoRetry == nil {
						// Close previous response body
						_ = httpResp.Body.Close()
						httpResp = httpRespRetry
						bodyBytes, errRead = io.ReadAll(httpResp.Body)
						if errClose := httpResp.Body.Close(); errClose != nil {
							log.Errorf("antigravity executor: close retry response body error: %v", errClose)
						}
						if errRead != nil {
							recordAPIResponseError(ctx, e.cfg, errRead)
							err = errRead
...
// executeImageNonStream performs a non-streaming request to the Antigravity API for image models.
// It bypasses the response translation to preserve raw image data structures.
func (e *AntigravityExecutor) executeImageNonStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (body []byte, err error) {
	token, updatedAuth, errToken := e.ensureAccessToken(ctx, auth)
	if errToken != nil {
		return nil, errToken
	}
	...
	translated = util.ApplyGemini3ThinkingLevelFromMetadataCLI(req.Model, req.Metadata, translated)
	translated = util.ApplyDefaultThinkingIfNeededCLI(req.Model, req.Metadata, translated)
	// Image models don't support thinking, but we run this for consistency in cleanup
	...
	for idx, baseURL := range baseURLs {
		...
		reporter.publish(ctx, parseAntigravityUsage(bodyBytes))
		// Return raw body without translation
		reporter.ensurePublished(ctx)
...
func (e *AntigravityExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (stream <-chan cliproxyexecutor.StreamChunk, err error) {
	if isImageModel(req.Model) {
		...
		go func() {
			defer close(ch)
			// Use specialized execution for image models to get raw response without translation loss
			respPayload, err := e.executeImageNonStream(ctx, auth, req, opts)
			if err != nil {
				ch <- cliproxyexecutor.StreamChunk{Err: err}
				return
			}
			// Convert raw Gemini response to OpenAI stream format
			streamChunk := convertNonStreamToStreamFormat(respPayload, req.Model)
			ssePayload := []byte("data: " + string(streamChunk) + "\n\n")
			ch <- cliproxyexecutor.StreamChunk{Payload: ssePayload}
	...
	for idx, baseURL := range baseURLs {
		...
		// DEBUG: Log full request for 400 error debugging (INFO level for visibility)
		log.Infof("antigravity ExecuteStream: model=%s, url=%s", req.Model, httpReq.URL.String())
		log.Infof("antigravity ExecuteStream: request payload:\n%s", string(translated))
		...
		if httpResp.StatusCode < http.StatusOK || httpResp.StatusCode >= http.StatusMultipleChoices {
			...
			if httpResp.StatusCode == http.StatusUnauthorized {
				log.Debugf("antigravity executor: received 401 on steam request, attempting token refresh")
				// Close the unauthorized response body immediately
				_ = httpResp.Body.Close()
				...
				if refreshedAuth, errRefresh := e.refreshToken(ctx, auth); errRefresh == nil {
					...
					if errReq == nil {
						if httpRespRetry, errDoRetry := httpClient.Do(httpReq); errDoRetry == nil {
							httpResp = httpRespRetry
							// Update metadata for the new response
							recordAPIResponseMetadata(ctx, e.cfg, httpResp.StatusCode, httpResp.Header.Clone())

							// Check status again for the retried response
							if httpResp.StatusCode < http.StatusOK || httpResp.StatusCode >= http.StatusMultipleChoices {
								bodyBytes, errRead := io.ReadAll(httpResp.Body)
								_ = httpResp.Body.Close()
								if errRead != nil {
									recordAPIResponseError(ctx, e.cfg, errRead)
									err = errRead // Return read error
									return nil, err
								}
								appendAPIResponseChunk(ctx, e.cfg, bodyBytes)
								// Fall through to error handling below
							} else {
								// Success on retry! Proceed to streaming
								goto startStreaming
							}
						}
					}
...
func FetchAntigravityModels(ctx context.Context, auth *cliproxyauth.Auth, cfg *config.Config) []*registry.ModelInfo {
	...
	// Create a base map with our mandatory models
	modelMap := map[string]gjson.Result{
		"gemini-3-pro-image": {},
	}

	token, updatedAuth, _ := exec.ensureAccessToken(ctx, auth)
	if updatedAuth != nil {
		auth = updatedAuth
	}
	...
	if token != "" {
		...
		for _, baseURL := range baseURLs {
			...
			httpReq, _ := http.NewRequestWithContext(ctx, http.MethodPost, modelsURL, bytes.NewReader([]byte(`{}`)))
			if httpReq == nil {
				continue
			}
			httpReq.Header.Set("Content-Type", "application/json")
			...
			if errDo == nil {
				bodyBytes, errRead := io.ReadAll(httpResp.Body)
				httpResp.Body.Close()
				if errRead == nil && httpResp.StatusCode == http.StatusOK {
	...
	for originalName := range modelMap {
		aliasName := modelName2Alias(originalName)
		registerNames := []string{aliasName}
		if originalName != aliasName && originalName != "" {
			registerNames = append(registerNames, originalName)
		}
		...
		for _, id := range registerNames {
			...
			cfg := modelConfig[id]
			// Fallback to alias-based config lookup if registering the raw name
			if cfg == nil && id == originalName {
			...
			// Look up support from static config
			if cfg != nil {
				if cfg.Thinking != nil {
...
func geminiToAntigravity(modelName string, payload []byte, projectID string) []byte {
	...
	// STRICT CLEANUP: Remove fields that cause 400 INVALID_ARGUMENT on v1internal
	template, _ = sjson.Delete(template, "request.safetySettings")
	template, _ = sjson.Delete(template, "request.generationConfig.topK") // Often causes issues with newer models if set

	// Special handling for gemini-3 models (including pro-low, pro-high, etc.)
	if strings.Contains(modelName, "gemini-3-") {
		// Gemini 3 models: Remove toolConfig entirely if no tools are present (avoids invalid mode errors)
		tools := gjson.Get(template, "request.tools")
		if !tools.Exists() || len(tools.Array()) == 0 {
			template, _ = sjson.Delete(template, "request.toolConfig")
		}

		// gemini-3-pro-low: Apply thinkingLevel: "LOW" explicitly
		if strings.Contains(modelName, "gemini-3-pro-low") {
			template, _ = sjson.Delete(template, "request.generationConfig.thinkingConfig.thinkingBudget")
			template, _ = sjson.Set(template, "request.generationConfig.thinkingConfig.thinkingLevel", "LOW")
		} else if strings.Contains(modelName, "gemini-3-pro-high") || strings.Contains(modelName, "gemini-3-pro") {
			// gemini-3-pro-high or gemini-3-pro: Apply thinkingLevel: "HIGH" (default for pro)
			if thinkingLevel := gjson.Get(template, "request.generationConfig.thinkingConfig.thinkingLevel"); !thinkingLevel.Exists() {
				template, _ = sjson.Delete(template, "request.generationConfig.thinkingConfig.thinkingBudget")
				template, _ = sjson.Set(template, "request.generationConfig.thinkingConfig.thinkingLevel", "HIGH")
			}
		} else {
			// Other gemini-3 models: convert thinkingLevel if needed
			if thinkingLevel := gjson.Get(template, "request.generationConfig.thinkingConfig.thinkingLevel"); thinkingLevel.Exists() {
		...
		// Always remove thinkingBudget for Gemini 3 if thinkingLevel is set
		if gjson.Get(template, "request.generationConfig.thinkingConfig.thinkingLevel").Exists() {
			template, _ = sjson.Delete(template, "request.generationConfig.thinkingConfig.thinkingBudget")
		}
		// Ensure temperature is valid float if present
		if temp := gjson.Get(template, "request.generationConfig.temperature"); temp.Exists() {
			template, _ = sjson.Set(template, "request.generationConfig.temperature", temp.Float())
		}
	} else {
		// Non-Gemini-3 models: set toolConfig
		template, _ = sjson.Set(template, "request.toolConfig.functionCallingConfig.mode", "VALIDATED")
		// Legacy thinking logic for older models
		if !strings.HasPrefix(modelName, "gemini-2.0-flash-thinking") { // Don't strip for thinking models
			if thinkingLevel := gjson.Get(template, "request.generationConfig.thinkingConfig.thinkingLevel"); thinkingLevel.Exists() {
				template, _ = sjson.Delete(template, "request.generationConfig.thinkingConfig.thinkingLevel")
				template, _ = sjson.Set(template, "request.generationConfig.thinkingConfig.thinkingBudget", -1)
			}
...
// convertNonStreamToStreamFormat converts an OpenAI non-stream response
// (choices[0].message.content) to stream format (choices[0].delta.content)
// This is needed for image models which return non-stream responses
// but need to be presented as stream chunks to the frontend.
func convertNonStreamToStreamFormat(payload []byte, model string) []byte {
	// Parse the non-stream response
	parsed := gjson.ParseBytes(payload)
	if !parsed.Exists() {
		log.Debugf("convertNonStreamToStreamFormat: payload does not parse as JSON")
		return payload
	}

	log.Debugf("convertNonStreamToStreamFormat: raw payload (first 500 chars): %s", string(payload)[:min(500, len(payload))])

	// 1. Try extracting content from OpenAI format (choices[0].message)
	// This happens if sdktranslator was used and successful (though unlikely for images now)
	message := parsed.Get("choices.0.message")
	var role, content string
	var imageURLs []interface{}

	if message.Exists() {
		log.Debugf("convertNonStreamToStreamFormat: found OpenAI format (choices.0.message)")
		role = message.Get("role").String()
		content = message.Get("content").String()
		if images := message.Get("images"); images.Exists() && images.IsArray() {
		...
		// 2. Try extracting from Gemini raw format (candidates[0].content.parts)
		// This is the expected path when using executeImageNonStream
		log.Debugf("convertNonStreamToStreamFormat: trying Gemini format")
		candidate := parsed.Get("response.candidates.0")
		if !candidate.Exists() {
			candidate = parsed.Get("candidates.0") // Try direct candidates (standard Gemini)
			log.Debugf("convertNonStreamToStreamFormat: tried candidates.0, exists: %v", candidate.Exists())
		} else {
			log.Debugf("convertNonStreamToStreamFormat: found response.candidates.0")
		}

		if candidate.Exists() {
			role = candidate.Get("content.role").String()
			parts := candidate.Get("content.parts")
			log.Debugf("convertNonStreamToStreamFormat: role=%s, parts.IsArray=%v, parts.count=%d", role, parts.IsArray(), len(parts.Array()))
			if parts.IsArray() {
				for i, part := range parts.Array() {
					log.Debugf("convertNonStreamToStreamFormat: part[%d] keys: text=%v, inlineData=%v, fileData=%v",
						i, part.Get("text").Exists(), part.Get("inlineData").Exists(), part.Get("fileData").Exists())
					...
					if inline := part.Get("inlineData"); inline.Exists() {
						...
						log.Debugf("convertNonStreamToStreamFormat: found inlineData, mime=%s, dataLen=%d", mime, len(data))
						if mime != "" && data != "" {
							// For inline base64 data, we must provide the full data URI
							url := "data:" + mime + ";base64," + data
							imageURLs = append(imageURLs, map[string]interface{}{
								"image_url": map[string]interface{}{
									"url":    url,
									"detail": "auto", // Add detail field just in case
								},
							})
						}
					}
					// Handle fileData if present (less common for direct generation but possible)
					if fileData := part.Get("fileData"); fileData.Exists() {
						if uri := fileData.Get("fileUri").String(); uri != "" {
							log.Debugf("convertNonStreamToStreamFormat: found fileData, uri=%s", uri)
							// Return URI directly
							imageURLs = append(imageURLs, map[string]interface{}{
								"image_url": map[string]interface{}{
									"url": uri,
			...
			log.Debugf("convertNonStreamToStreamFormat: no candidate found, returning raw payload")
			return payload
		}
	}

	log.Debugf("convertNonStreamToStreamFormat: final imageURLs count: %d, content length: %d", len(imageURLs), len(content))
	// Build stream format response with delta instead of message
	id := parsed.Get("id").String()
	if id == "" {
		id = "chatcmpl-image-" + uuid.New().String()
	}
	...
	if len(imageURLs) > 0 {
		delta := streamResp["choices"].([]map[string]interface{})[0]["delta"].(map[string]interface{})
		delta["images"] = imageURLs
	}

	result, err := json.Marshal(streamResp)
	if err != nil {
		return payload
	}
	return result
