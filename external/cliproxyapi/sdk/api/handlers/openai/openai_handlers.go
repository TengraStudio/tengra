...
func (h *OpenAIAPIHandler) ChatCompletions(c *gin.Context) {
	...
	if isResp, reason := shouldTreatAsResponsesFormat(rawJSON); isResp {
		fmt.Printf("[Proxy] Treating as ResponsesFormat. Reason: %s\n", reason)
...
func shouldTreatAsResponsesFormat(rawJSON []byte) (bool, string) {
	...
	if gjson.GetBytes(rawJSON, "input").Exists() {
		return true, "input field exists"
	}
	if gjson.GetBytes(rawJSON, "instructions").Exists() {
		return true, "instructions field exists"
	}
	return false, ""
}
